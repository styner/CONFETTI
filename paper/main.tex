...
well as prediction of later cognitive and behavioral outcomes. We compare
...
White matter (WM) microstructure analysis from diffusion MRI data typically reduces 
...
Fiber-bundle methods such our atlas based fiber parametrization approach ~\cite{ref_cohort} 
or similar ones such the Automated Fiber Quantification
family~\cite{ref_yeatman} generate along-tract, per-node diffusion metric profiles. Current profile analysis methods focsu on per-tract analyses, and no all-tract/whole-brain methods exist that operates on these 1D profiles
...
of 24-month cognitive and behavioral outcomes from 6-month
...
pattern predicts 24-month cognitive outcomes from 6-month imaging.
...
importance metric is the held-out AUC (or difference in held-out MAE for regression) when a
...
for attribution has never seen the test subjects.
%; the refit-all model
...
prospective regression we predict scores from two behavioral/cognitive assessments measured at 24 months of age: Vineland-III Adaptive Behavior Composite, Communication, Daily Living Skills, Motor, and Socialization scores; as well as Bayley-4 Cognitive standard, Expressive and
...
three CONFETTI models land a bit lower, between $0.913$ and $0.953$, with the
Graph U-Net at the upper end. The gap (at this sample size) is
...
\begin{figure}[tb]
  ...
  the AC olfactory at rank 1; the baselines rank the MCP 
   and CC Lateral Genu tracts highest. The MCP and CC Lateral Genu tracts appear in $\geq 5/6$ top-10 lists; AC olfactory is CONFETTI-specific.}
  \label{fig:rank}
\end{figure}
...
in DS cohorts~\cite{ref_cohort,ref_saini2025white,ref_carducci}, the group differences invert direction for FA, ODI, and NDI. 
This effect inversion cannot be captured by tract means and is likely one of the reasons the baselines do not rank the AC olfactory highly.   
...
We applied CONFETTI to a multi-output regression on all 24-month
outcomes, training jointly (z-score targets, smooth L1 loss, per-target NaN masking). Table~\ref{tab:reg} shows held-out Pearson $r$ for
...
outcomes and is within $0.05$ of
XGBoost on the rest. The largest absolute Pearson $r$ is on Bayley COG
and Vineland Motor.
...
\midrule
Vineland ABC        & $0.37\,(.11)$ & $0.46\,(.12)$ & $\mathbf{0.53\,(.11)}$ \\
Vineland Communication               & $0.44\,(.14)$ & $0.53\,(.13)$ & $\mathbf{0.54\,(.16)}$ \\
Vineland Daily Living Skills         & $0.32\,(.19)$ & $0.48\,(.12)$ & $\mathbf{0.51\,(.13)}$ \\
Vineland Motor                       & $0.64\,(.13)$ & $\mathbf{0.68\,(.11)}$ & $0.66\,(.12)$ \\
Vineland Socialization               & $0.18\,(.12)$ & $0.22\,(.13)$ & $\mathbf{0.39\,(.20)}$ \\
...
Figure~\ref{fig:heat} shows CONFETTI's per-24-month-target OOF tract permutation
importance for the Graph U-Net head. AC olfactory is the top-attributed
tract for all outcomes. The single largest cell in the heatmap
is AC olfactory $\times$ Bayley Cognitive where permuting AC olfactory tract features
increases the held-out MAE on the Bayley Cognitive score by approximately
$0.80$. Aggregating across the three CONFETTI architectures, AC
olfactory ranks first in $22$ of $24$ (target $\times$ architecture)
combinations. 

It worthwhile to note that the left and right optic tract are
consistent secondary contributors. These tracts show an opposite directional signature to the AC olfactory tract: higher FA and lower RD/ODI indicating a "more coherent / better organized microstructure" pattern. This finding is biologically surprising, possibly reflecting altered visual-system development in DS.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.78\linewidth]{fig_target_heatmap.pdf}
  \caption{CONFETTI per-target tract attribution for the 6-month-diffusion$\rightarrow$24-month-behavior/cognition
  predictive regression (Graph U-Net). Cell color is the held-out MAE
...
gestational age shows a predictive signal for the Bayley Cognitive score ($+0.264$ MAE), and sex for the Daily Living score ($+0.212$). The AC olfactory's attribution
significantly exceeds covariate contributions.
...
olfactory tract at rank 1, while the tract mean methods put the middle cerebellar peduncle at rank 1 with
AC olfactory only mid-list. The follow-up experiments establish
...
Our novel fiber neighborhood graph based analysis framework called \textbf{CONFETTI} recovers spatial signals that traditional per-tract-mean baselines do not reveal with robust, novel findings of the relevance of the AC olfactory tract in DS. 
...
Saini, F, Ivain, P, Idris, M, Wells, J , Bianchi, L , Tamayo‐Elizalde, M , Dell'Acqua, F , Strydom, A: White matter trajectories in Down syndrome and Alzheimer's disease: Insights from diffusion tensor–based morphometry. Alzheimer's \& Dementia \textbf{6} (21) , e70382 (2025)
